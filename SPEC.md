...
boundaries in the otherwise-flat track list:
...
The divider is a **visual separator only — its children are empty**.
Browsing `disc:{album_id}:{N}` returns zero items; `childCount` is a display
hint, not a promise of browsable children. This is deliberate: a control
point (verified on Linn) builds a play queue by enqueuing the album
listing's track `<item>`s *and* recursing into any child `<container>`. If
the divider returned its disc's tracks, every disc would be enqueued twice
(once as flat items, once via recursion — the "Disc 1 queued twice" bug).
Empty children means "play album" recurses each divider to nothing, so the
full album queues continuously with each track exactly once, while the
separator still renders inline. Single-disc albums emit **no** divider.
...
20. Multi-disc albums (`MAX(disc_num) > 1`) emit:
    ...
    - **disc-divider containers** (`disc:{album_id}:{disc}` with title
      ...
      rendering. The divider is a **visual separator only**: its child list
      is empty (browsing `disc:{album_id}:{disc}` returns nothing). This is
      what stops "play album" from queuing each disc twice — a control point
      enqueues the listing's items *and* recurses its child containers, so a
      divider that returned its disc's tracks double-queued every disc
      (§7.2.1). MinimServer ships the same inline pattern (§7.2, §14).
...
- **Multi-disc albums**: identical `(effective_album_artist, album,
  ...
  divider is a **visual separator only: its child list is empty**. Do
  **not** make it return its disc's tracks — that is exactly what caused
  the "Disc 1 queued twice" bug. A control point builds a play queue by
  enqueuing the album listing's track `<item>`s *and* recursing into any
  child `<container>`; a divider with real children gets recursed and
  re-enqueues the whole disc. Empty children keep `<item>` + `<container>`
  from ever describing the same tracks twice, so "play album" queues each
  track once and "play disc" simply isn't offered. Single-disc albums skip
  both — `dc:title` is **never** modified for disc info, so single-disc
  browsing is unchanged. When tags carry album names like `"Album [Disc 1]"`,
  the result is two separate albums; fix this in tags. Automatic disc
  merging based on parsed-out suffixes is **not implemented** (intentional —
  the parsing rules are too lossy). Non-order-preserving control points
  (VLC, foobar2000, MediaMonkey) may bunch the divider containers separately
